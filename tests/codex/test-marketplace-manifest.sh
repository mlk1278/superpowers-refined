...
assert_equal(marketplace.get("name"), "toolbelt-dev", "marketplace name")
assert_equal(
...
matching_plugins = [plugin for plugin in plugins if plugin.get("name") == "toolbelt"]
assert_equal(len(matching_plugins), 1, "toolbelt plugin entry count")
