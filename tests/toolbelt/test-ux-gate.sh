...
assert_contains "$skill" "Record why any excluded dimension cannot vary" "excluded dimensions are recorded"
assert_contains "$skill" "a conflict to surface, not obey" "plan-imposed matrices are surfaced"
...
assert_contains "$skill" "where would a designer wince" "design pass asks the wince question"
assert_contains "$skill" "a severity (blocker, should, nit), the image reference" "finding format"
