...
`delivery` selects one coherent delivery slice, creates an isolated worktree, resolves routes, and runs subagent-driven development. A materially user-visible slice runs the UX gate after task reviews and before SDD's broad final review. That broad review is the slice gate. Branch completion creates one PR; the PR monitor owns CI, configured review, fixes, merge, and confirmation. The monitor may run in the background while the orchestrator starts the next independent slice — one monitored PR at a time; its return is processed when it arrives, and in-flight lanes rebase onto the updated base before their final review. The project's issue tracker is reconciled only when the plan is linked to one, then the worktree, branch, and scratch are removed.
