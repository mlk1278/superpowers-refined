...
- **New test file must be executable:** `chmod +x tests/toolbelt/test-interactive-design.sh` before committing, matching every sibling.
...
One PR: the skill without its brainstorming fork is unreachable, and the fork without the skill dangles — neither half is independently verifiable, and the version bump belongs to the release that contains both.
...
Create `tests/toolbelt/test-interactive-design.sh` copying the structure of `tests/toolbelt/test-ux-gate.sh` (same `assert_contains` helper, same `repo_root` resolution), `chmod +x`. One `assert_contains` per line — file — needle — description:
...
**Files:**
- Modify: `skills/brainstorming/SKILL.md` (HARD-GATE block at lines 10–13; "After the Design" section at lines 50–52; new section inserted between "After the Design" and "Visual Companion")
...
- `skills/brainstorming/SKILL.md` — `accepted the frontend-first offer` — HARD-GATE exception present
...
- `skills/brainstorming/SKILL.md` — `toolbelt:interactive-design` — fork names the skill
- `skills/writing-specs/SKILL.md` — `interactive-design` — entry gate acknowledges the path
...
- [ ] **Step 3: Make the three brainstorming changes and one writing-specs change**
...
3. In "After the Design", extend the handoff sentence with the fork: accepted frontend-first offer → invoke `toolbelt:interactive-design`; otherwise → writing-specs, unchanged. Keep "Do NOT invoke any other skill. writing-specs is the next step." scoped to the declined branch; each branch names exactly one next skill.
4. Inside writing-specs' `<ENTRY-GATE>` block, after the direct-entry sentence, append D7.
...
Run: `git grep -n '"version": "7.6.0"' -- .claude-plugin .codex-plugin package.json | wc -l` → expected `3`; `grep -c '"version": "7.6.0"' .claude-plugin/marketplace.json` → expected `1`.
...
## Post-merge (human steps, not plan tasks)

The spec's manual acceptance tests run after release: refresh both plugin caches (uninstall-first for Claude Code — install is a no-op at an unchanged version), fresh session per harness; verify the offer fires for a UI-heavy feature after intent-level approval, does not fire for backend-only work, and run one accepted-path drill (skill invoked, fixture + ledger in the same edit, reconciliation, handoff to writing-specs) plus one declined run confirming the offer is not repeated.
