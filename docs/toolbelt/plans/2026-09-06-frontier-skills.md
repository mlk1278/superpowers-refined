...
- No new npm dependencies in the plugin. The capture script resolves `playwright` from the consuming project: `createRequire(join(projectRoot, 'package.json')).resolve('playwright')`, then `process.env.PLAYWRIGHT_MODULE`, then `createRequire(import.meta.url).resolve('playwright')` for a global install. When none resolves: print `ux-capture: cannot resolve 'playwright'. Install it in the project (npm i -D playwright) or set PLAYWRIGHT_MODULE=/path/to/playwright` and exit 2.
...
- Playwright may be absent on the machine running the tests. `test-ux-capture.sh` (Task 12) must detect that with the script's own exit 2 and print `SKIP` with exit 0, never a false PASS.
...
| `element-overflow` | should | a visible leaf element's `right` or `bottom` exceeds the rect of its nearest ancestor with a non-auto width or height whose computed `overflow-x`/`overflow-y` is not `auto` or `scroll`, by more than 1px, unless the element or an ancestor matches an `allowOverflow` selector |
...
| `unclickable` | blocker | for each visible `button, a[href], [role=button], input, select, textarea`, `document.elementFromPoint(cx, cy)` is neither the control, nor inside it, nor an ancestor of it |
| `tap-target` | nit | `viewport.width < 500` and an interactive control's rect is under 44×44 CSS px |
| `layout-shift` | should | sum of `layout-shift` entries with `hadRecentInput === false` from navigation to capture exceeds 0.1; `clsSources` lists each source node's `tagName.className` |
...
"Visible" means `getBoundingClientRect()` has width and height > 0, computed `visibility` is not `hidden`, and `display` is not `none`. "Leaf" means no element children.
...
| 2 | Bundled UX capture script with mechanical checks and diff; ux-gate, implementer smoke, baseline, and routing rewritten around it | 12–15 | 1 | `bash tests/toolbelt/test-ux-capture.sh` (PASS or SKIP); `bash tests/toolbelt/test-ux-gate.sh`; `bash tests/toolbelt/test-agent-routing.sh` |
...
| discipline | 2–3 | serial-1 | using-toolbelt, hooks/session-start, verification-before-completion, receiving-code-review, systematic-debugging, test-driven-development | disjoint from design, delivery-chain, orchestration; owns no test file |
...
| delivery-chain | 6–7 | serial-1 | finishing-a-development-branch, dispatching-parallel-agents, using-git-worktrees, requesting-code-review (+ code-reviewer.md), tests/toolbelt/test-completion-contract.sh, test-worktree-baseline.sh, test-worktree-source-ref.sh | disjoint from the others; keeps test-reviewer-context.sh and test-execution-tracks.sh needles verbatim |
| orchestration | 8–10 | serial-1 | writing-plans (+ execution-tracks.md, − plan-document-reviewer-prompt.md), SDD (+ parallel-tracks.md, three prompts), delivery, pr-monitor, agent-routing, quick-task, tests/toolbelt/test-writing-plans.sh, test-execution-tracks.sh, test-final-review-gate.sh, test-fix-loop.sh, test-reviewer-context.sh, test-delivery.sh, test-pr-monitor.sh, test-quick-task.sh | disjoint from the others; owns every test file it touches |
...
```bash
git add skills/using-toolbelt/SKILL.md hooks/session-start skills/verification-before-completion/SKILL.md
...
In `tests/toolbelt/test-interactive-design.sh`, change the needle `The offer MUST be its own message` to `The offer is its own message`.
...
- Modify: `docs/toolbelt/specs/2026-09-06-frontier-skills-design.md` (Testing section, one bullet)
...
```bash
git add skills/interactive-design/SKILL.md skills/interactive-design/iteration-mode.md tests/toolbelt/test-interactive-design.sh docs/toolbelt/specs/2026-09-06-frontier-skills-design.md
...
`code-reviewer.md` at most 500 words. Keep every needle string verbatim, the read-only rules, the git fallback commands, What to Check, the output format headings, and the placeholder list. Remove "Acknowledge what was done well ... helps the implementer trust". Keep Strengths as a section heading. The Calibration paragraph is replaced in Task 18; here reduce it to "Categorize issues by actual severity. Not everything is Critical."
...
Run: `test "$(wc -w < skills/using-git-worktrees/SKILL.md)" -le 750 && test "$(wc -w < skills/requesting-code-review/SKILL.md)" -le 350 && test "$(wc -w < skills/requesting-code-review/code-reviewer.md)" -le 500 && bash tests/toolbelt/test-worktree-baseline.sh && bash tests/toolbelt/test-worktree-source-ref.sh && bash tests/toolbelt/test-reviewer-context.sh && bash tests/toolbelt/test-execution-tracks.sh`
...
**Gotchas:**
- `test-execution-tracks.sh` `$plans` needles: `No file is created or modified by two concurrent tracks`, `contract-freeze`, `Every fork closes with a mainline integration task`, plus two multi-line needles at lines 9–13 (read them). These move to `execution-tracks.md`: add `tracks="$repo_root/skills/writing-plans/execution-tracks.md"` and point the `$plans` track needles at it. Task 9 edits the same test's `$sdd` needles; sequential in track `orchestration`.
...
```bash
git add -A skills/writing-plans tests/toolbelt/test-writing-plans.sh tests/toolbelt/test-execution-tracks.sh
...
**Gotchas:**
- `test-execution-tracks.sh` `$sdd` needles: `At most 3 tracks run concurrently`, `A textual conflict is a plan defect`, `Dispatch multiple implementation subagents into the same worktree`, `Parallelize tracks the plan did not declare`, one multi-line needle at line 14–16, and `assert_not_contains 'Dispatch multiple implementation subagents in parallel (conflicts)'`. The track-mechanics needles move to `parallel-tracks.md` (add `$sdd_tracks`); the two "Never" needles stay in `SKILL.md` under `## Ownership rules`.
- `test-final-review-gate.sh` needles all stay verbatim (read all 16 plus the three `assert_not_contains`). `test-fix-loop.sh` needles all stay verbatim. `test-reviewer-context.sh` needles all stay verbatim.
...
`SKILL.md` at most 1,500 words, frontmatter unchanged. Keep every contract in The Process, Model Selection, Handling Implementer Status, The Fix Loop (including both routes, the ledger lines, adjudication), Verification Scope, Constructing Reviewer Prompts, File Handoffs, Durable Progress, Prompt Templates, Integration, and `## Ownership rules` (the six "Never" bullets, under that heading). Replace `## Parallel Tracks` with: "Active only when the plan declares a top-level `## Execution Tracks` section; otherwise this skill is serial. Read [parallel-tracks.md](parallel-tracks.md) when it does." `parallel-tracks.md` holds Wave execution, Working directories, Drift log, Ledger, and Failure semantics verbatim.

- [ ] **Step 3: Light pass on the three prompts**

Within the rewrite rules, keep every placeholder, section heading, and needle string. `task-reviewer-prompt.md` at most 650 words: collapse the three "does not override requirements, suppress findings, or set severity" restatements to one (the `[REVIEW_NUANCE]` section keeps `does not override requirements,`); keep Calibration as is (Task 18 replaces it). `implementer-prompt.md` at most 550 words: no content change beyond prose. `re-review-prompt.md` at most 420 words.

- [ ] **Step 4: Update tests**

In `test-execution-tracks.sh`, add `sdd_tracks="$repo_root/skills/subagent-driven-development/parallel-tracks.md"` and point `At most 3 tracks run concurrently`, `A textual conflict is a plan defect`, and the multi-line wave needle at it. Leave the other two test files unchanged; run them.

- [ ] **Step 5: Verify**

Run: `test "$(wc -w < skills/subagent-driven-development/SKILL.md)" -le 1500 && test "$(wc -w < skills/subagent-driven-development/task-reviewer-prompt.md)" -le 650 && for t in test-execution-tracks test-final-review-gate test-fix-loop test-reviewer-context; do bash tests/toolbelt/$t.sh || exit 1; done`
Expected: four `PASS` lines.
...
- `gates_under_80_words` — for every `skills/*/SKILL.md`, each `<HARD-GATE>…</HARD-GATE>` and `<ENTRY-GATE>…</ENTRY-GATE>` block extracted with `sed -n` has `wc -w` ≤ 80
...
Run: `for t in tests/toolbelt/*.sh tests/hooks/*.sh; do echo "== $t"; bash "$t" || exit 1; done`
...
- Produces: `ux-capture <matrix.json> --out <dir> [--smoke] [--pathway <name>]...` exit 0 clean, 1 findings at `should` or above, 2 cannot run; `mechanical.json` per the Data Model with `diff: null`, `files.filmstrip: null`, `files.diffCrop: null`; Task 13 adds `--baseline`, `--video`, filmstrips, axe
...
- `skip_without_playwright` — run the script on the fixture matrix with `PLAYWRIGHT_MODULE=/nonexistent` and no project playwright, in a temp dir — exit 2 and stderr contains `cannot resolve 'playwright'`; the test then checks whether `node -e "require.resolve('playwright')"` succeeds from the repo root or `$PLAYWRIGHT_MODULE` is set; if neither, print `SKIP - playwright unavailable` and exit 0
- `smoke_finds_fixture_defects` — serve `fixtures/ux-capture/`, rewrite `baseUrl` into a temp matrix, run `--smoke --out $tmp/out` — exit 1; `mechanical.json` parses; contains a `checks[]` entry `element-overflow` with selector containing `overflow-child`, `unclickable` with selector containing `Covered`, and, for the dark capture, `theme-leak` with selector containing `hardcoded-white`; every still named per Data Model exists
- `smoke_uses_first_viewport_only` — same run — every entry has `width == 375`

The fixture `index.html`: a `<div class="card" style="width:200px;overflow:hidden">` containing a `<span class="overflow-child" style="display:inline-block;width:300px">`; a `<button>Covered</button>` under a `position:absolute` full-width overlay `div`; a `<div class="hardcoded-white" style="background:#fff;width:200px;height:200px">`; `<html class="">` with CSS `.dark body{background:#111;color:#eee}`; a `<button id="open">Open</button>` that toggles a `<div id="panel" hidden>` (used by Task 13's motion test). `matrix.json`: `theme.mode: "class"`, `target: "html"`, both themes, the three default viewports, one pathway `home` at `/index.html` with steps `open` (default) and `panel` (`action: click`, `selector: #open`, `motion: true`, `waitFor: #panel:not([hidden])`).
...
`#!/usr/bin/env node`, ESM. Structure: `parseArgs` (matrix path, `--out`, `--smoke`, `--pathway` repeatable, `--help`; `--baseline` and `--video` accepted and ignored until Task 13); `resolvePlaywright()` per Global Constraints; `loadMatrix(path)` applying defaults; `applyTheme(context|page, theme, matrix.theme)` per mode; `mechanicalChecks()` — a single function serialised into `page.evaluate` returning `{checks, cls, clsSources, fonts}` implementing every row of the checks table except `axe`; the capture loop `for viewport → for theme → for pathway → newContext → addInitScript(layout-shift observer) → goto(baseUrl + path, networkidle) → for step: action, waitFor (15 s, `step-failed` on timeout), settle, still, crops, mechanical, entry`; reference screens after pathways; write `mechanical.json`; exit code from the max severity across entries. Console and request listeners are attached per page and drained per step.

- [ ] **Step 4: Run the test to verify it passes**

Run: `bash tests/toolbelt/test-ux-capture.sh`
Expected: `PASS` (or `SKIP - playwright unavailable` on a machine without Playwright; then run once with `PLAYWRIGHT_MODULE` pointing at any installed Playwright to confirm `PASS`, and paste that run in the report).
...
Add assertions:
- `filmstrip_for_motion_step` — the smoke run from Task 12 is replaced by a full run (`--out $tmp/full`) — file `home-panel-default-375-light-filmstrip.png` exists and is larger than 1 KB
...
Add `diffAgainstBaseline(page, headPng, basePng)`; `filmstrip(page, step, tag)`; `recordVideo` on context creation when `--video`; the axe branch; `--pathway` filtering (already parsed) and the `diff` and `files` fields per Data Model.
...
- Modify: `skills/ux-gate/agents/openai.yaml`
...
- `openai.yaml` `short_description` must stay within 25–64 characters.
...
```bash
git add skills/ux-gate/SKILL.md skills/ux-gate/agents/openai.yaml tests/toolbelt/test-ux-gate.sh docs/ADOPTING-IN-A-PROJECT.md README.md
...
**Gotchas:**
- `test-agent-routing.sh:162` pins every non-reviewer bundled role's fallback as codex sol; leave those roles alone. Reviewer expectations pass `--author-harness claude`, so the new Claude fallback is filtered and `fallbacks: []` stays true. Add: `bundled gate specialty, author claude` → codex sol high, `fallbacks: []`, `source: bundled:reviewer-specialty`; `bundled gate specialty, author codex` → claude fable-5-1 high, `source: bundled:reviewer-specialty`; same pair for `ux`. Copy the `assert_route` shape from line 173.
...
Add the four `assert_route` cases above to `test-agent-routing.sh` and the two needles to `test-delivery.sh`.
...
`defaults.json`: the Data Model additions; reviewer fallback to `fable-5-1`. agent-routing Roles table: `reviewer` row lists specialties `code`, `spec`, `plan`, `ux`, or `gate`.
...
**Interfaces:**
- Consumes: Task 8's writing-plans
- Produces: the `**Review:** immediate | gate` field and the review-class rule text SDD (Task 19) reads
...
Task Structure template: after the `**Interfaces:**` block add `**Review:** immediate | gate`. Task Right-Sizing: append the paragraph from Spec C4 verbatim, beginning `**Review class.** A task is \`immediate\` when any of these hold:`. Plan Review Gate: add the bullet `- **Review class** — every task stamped, each \`gate\` stamp consistent with the review-class rule`. Execution Handoff's closing sentence becomes: "Delivery runs toolbelt:subagent-driven-development per boundary: fresh subagent per task, immediate review or a batched gate by review class, broad whole-branch review at the end."
...
Run: `bash tests/toolbelt/test-review-classes.sh && bash tests/toolbelt/test-reviewer-context.sh && bash tests/toolbelt/test-fix-loop.sh && bash tests/toolbelt/test-word-counts.sh`
Expected: all pass.
...
**Interfaces:**
- Consumes: Task 18's prompt and self-check; Task 14's ux-gate contract; Data Model ledger lines. The `**Review:**` field name is fixed by the spec's Data Model, not by Task 16; SDD reads it by that name.
...
**Gotchas:**
- `test-delivery.sh` `$workflow` needles: `\`brainstorming\` and \`writing-plans\``, `one coherent delivery slice`, a multi-line needle at line 401, `implementation report and review-package path`, `without independently rereading the implementation or verification output`, `No separate resume state machine`, `one pr-monitor per chain`; `test-workflow-summary.sh`: `quick-task`, `delivery`, the planning needle, `redesign the capability, not this summary`, and a forbidden-terms grep. All stay.
- WORKFLOW.md ceiling 320.

- [ ] **Step 1: Write the failing test**

Add to `test-workflow-summary.sh`: `reviewed by class` and `The UX gate runs beside the final gate`.

- [ ] **Step 2: Run to verify it fails**

Run: `bash tests/toolbelt/test-workflow-summary.sh`
Expected: `not ok` on `reviewed by class`.

- [ ] **Step 3: Implement**

WORKFLOW.md delivery paragraph: replace "A materially user-visible slice runs the UX gate after task reviews and before SDD's broad final review, which is the slice gate." with "Tasks are reviewed by class: `immediate` tasks get a task review on completion; `gate` tasks self-check with evidence and are reviewed together at a gate — track end, four tasks, 1,500 lines, or the boundary's final review, whichever comes first — by one top-tier cross-harness reviewer with one fix wave. The UX gate runs beside the final gate, which is the slice gate." AGENTS-SNIPPET.md: one line — "Tasks carry a review class: `immediate` tasks are reviewed on completion, `gate` tasks self-check and are reviewed together at a gate." README: in the skills list, after subagent-driven-development, no new bullet; update the writing-plans bullet to mention review classes if it describes task structure, else leave.
