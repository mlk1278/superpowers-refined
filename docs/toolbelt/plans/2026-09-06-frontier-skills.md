...
| `overlap` | should | two visible block leaves in the same nearest positioned layer whose rects intersect by more than 4px on both axes, unless either matches `allowOverlap`; exclude overlay/background pairs but check siblings inside fixed, absolute, and sticky containers |
...
Diff: any changed pixels surviving the tolerance/noise filter make the image `changed`; there is no whole-image ratio cutoff. `ratio` = changed pixels ÷ (max(widthA, widthB) × max(heightA, heightB)); `box` = `[x, y, w, h]` in image pixels of the head still; images of different dimensions are `changed` with `box` covering the head still.
