...
| `axe:<rule-id>` | critical/serious → `should`; moderate/minor → `nit` | one entry per `@axe-core/playwright` violation when the module resolves from the project root (full runs only): `selector` is the violation's first target, `text` its `help` string, plus `impact` and `nodes` (count); the raw violations array is also kept in the entry's `axe` field |
