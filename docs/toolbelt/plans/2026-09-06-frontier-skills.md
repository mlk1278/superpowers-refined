...
| `overlap` | should | two visible leaves, neither an ancestor of the other, neither having computed `display` `inline` (wrapped inline boxes have misleading union rects), and neither itself nor any ancestor positioned `absolute`, `fixed`, or `sticky` (overlays and sticky bars overlap content by design), whose precomputed `getBoundingClientRect()` rects intersect by more than 4px on both axes, unless either matches an `allowOverlap` selector; candidates are capped at 1,500 leaves and the entry records `overlapTruncated: true` when the cap applied |
...
Diff: `ratio` = changed pixels ÷ (max(widthA, widthB) × max(heightA, heightB)); `box` = `[x, y, w, h]` in image pixels of the head still, `null` when `unchanged` or `new`; images of different dimensions are `changed` with `box` covering the head still. A diff crop of a `fullPage` still uses `page.screenshot({clip, fullPage: true})` so a box below the fold is captured rather than clamped.
