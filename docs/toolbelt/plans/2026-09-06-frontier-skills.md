...
The schema and operator examples also ship in `skills/ux-gate/matrix.md`
(750-word ceiling), linked from ux-gate and the SDD smoke handoff. Validate
nonempty themes, viewports, pathways, and step lists, unique output names,
valid actions, and positive viewport dimensions before launching a browser.
A named auth file that is missing is a cannot-run error; do not fall back
to anonymous capture. `matrix.fonts` names required web fonts with loaded
FontFace declarations, not generic fallback families.

Optional step fields: `key` for `press`; string `value` for `fill`;
`expectFocus` (CSS selector) to assert the focused element; `capture: false`
to execute and check a step without images. Global keyboard steps have no
selector and use `page.keyboard.press(key)`.
...
| `element-overflow` | should | for each axis, find the visible leaf's nearest ancestor whose computed overflow on that axis is not `visible`: `auto` or `scroll` means the element can be scrolled into view — no finding on that axis; `hidden` or `clip` means compare, and report when the leaf's `right` (x axis) or `bottom` (y axis) exceeds that ancestor's rect by more than 1px; with no such ancestor, only `right` is compared, against the viewport's `clientWidth`; skipped when the element or an ancestor matches an `allowOverflow` selector |
...
| `overlap` | should | two visible block leaves in the same nearest positioned layer whose rects intersect by more than 4px on both axes, unless either matches `allowOverlap`; exclude overlay/background pairs but check siblings inside fixed, absolute, and sticky containers; cap candidates at 1,500 and record `overlapTruncated: true` when exceeded |
| `unclickable` | blocker | for each visible `button, a[href], [role=button], input, select, textarea` whose centre `(cx, cy)` lies inside the viewport, `document.elementFromPoint(cx, cy)` is neither the control, nor inside it, nor an ancestor of it. Skipped: controls outside the viewport; controls inside `[aria-hidden="true"]` or `[inert]`; controls with computed `pointer-events: none` or `opacity: 0`, or a rect of 1×1 px or smaller (visually-hidden inputs); and, while a visible `[aria-modal="true"]` element or `dialog[open]` is present, every control outside it (a backdrop covering the page is the design; a closed dialog carrying `aria-modal` skips nothing) |
...
| `console-error` | should | a `console` message of type `error` or an uncaught `pageerror` during the step |
...
| `font-fallback` | should | fonts still loading after 3 s, or a required web-font family lacks a loaded FontFace declaration or fails `document.fonts.check` |
...
| `axe:<rule-id>` | critical/serious → `should`; moderate/minor → `nit` | one entry per `@axe-core/playwright` violation when the module resolves from the project root (every non-`--smoke` run, `--pathway` included): `selector` is the violation's first target, `text` its `help` string, plus `impact` and `nodes` (count); the raw violations array is also kept in the entry's `axe` field |

"Visible" means `getBoundingClientRect()` has width and height > 0, computed `visibility` is not `hidden`, and `display` is not `none`. "Leaf" means no element children. `checks[].selector` is `TAG#id.class1.class2` (tag upper-case; `#id` and classes only when present, classes in DOM order) and every check entry also carries `text`: the element's `textContent` trimmed to 40 characters. Document-level checks (`page-overflow`, `font-fallback`, `step-failed`, `console-error`, `failed-request`) carry `selector: null` and `text: null`, plus `message` for `step-failed`, `console-error`, and `failed-request`. `clsSources` uses the same selector format. `console[]` entries are the message strings of `console` messages of type `error` only; `failedRequests[]` entries are `"<status> <url>"` or `"failed <url>"`; `fonts` is `document.fonts.status` (`loaded` | `loading` | `error`). `axe` is `"skipped"` under `--smoke`, `"unavailable"` when the module does not resolve, `"error"` when `analyze()` threw (message on stderr), else the raw violations array. Reference-screen entries have `tag: reference-<n>-<width>-<theme>`, `state: "default"`. Paths: `storageState` resolves relative to `--project-root`; `--out` and `--baseline` resolve relative to the working directory. Conventions: the boundary gate captures into `.toolbelt/ux/head/` (each fix round into `.toolbelt/ux/head-<n>/`, diffed against the previous round), the baseline lives at `.toolbelt/ux/baseline/`, and a task's smoke run writes `.toolbelt/ux/smoke/task-<N>/`; all ignored scratch.
...
Actions: `click`, `hover`, `focus`, `fill` call the Playwright locator method of that name (`fill` takes `value`); `press` takes `key` and uses the locator when a selector is provided, otherwise `page.keyboard`; `expectFocus` checks the active element after the action; `scroll` calls `locator.scrollIntoViewIfNeeded()` then `element.scrollIntoView({block: 'start'})`. Each step performs its action once per context. `capture: false` skips images, crops, and filmstrips while retaining actions, assertions, and mechanical checks. Order per step: action → filmstrip frames (motion steps only, animations enabled, at 0, 150, 400 ms after the action returns) → `waitFor` (15 s) → two animation frames → still (`animations: 'disabled'`, `caret: 'hide'`) → crops → mechanical checks. A `motion` step's reduced-motion frame comes from a second context with `reducedMotion: 'reduce'` that repeats the navigation and every earlier step's action on that pathway, then this step's action, and captures one frame 400 ms after it.

Crops: captured in a dedicated DPR-2 context (same viewport width and height, same theme, same navigation and step actions replayed), never by upscaling the still. A `crops` selector uses `locator.screenshot()` (Playwright scrolls it into view). A `-diff-crop.png` uses `page.screenshot({clip})` with `clip = {x: box[0]/dpr, y: box[1]/dpr, width: box[2]/dpr, height: box[3]/dpr}` where `dpr` is the head still's device scale factor and the 24px padding is applied in CSS px. For a viewport still the clip is viewport-relative, so the replayed page keeps the scroll position the step left it at (a `scroll` step's crop shows the scrolled viewport); only a `fullPage` still scrolls to the top first, where the clip is in document coordinates.

A failed requested diff/crop/filmstrip or installed axe scan produces a blocker; stderr alone is insufficient. Only an absent optional axe module may be unavailable.

Diff: any changed pixels surviving the tolerance/noise filter make the image `changed`; there is no whole-image ratio cutoff. `ratio` = changed pixels ÷ (max(widthA, widthB) × max(heightA, heightB)); `box` = `[x, y, w, h]` in image pixels of the head still, `null` when `unchanged` or `new`; images of different dimensions are `changed` with `box` covering the head still. A diff crop of a `fullPage` still uses `page.screenshot({clip, fullPage: true})` so a box below the fold is captured.
...
**Files:**
- Modify: `skills/ux-gate/scripts/ux-capture.mjs`
...
At most 950 words. The description triggers for new user flows, material interaction/layout/responsive changes, or an explicit request for UX review. Routine button-colour, copy, and isolated CSS edits do not invoke a model reviewer on their own; assess impact rather than line count. Follow Spec C3: link `matrix.md`, preflight, derive relevant pathways and states, run the bundled capture, handle mechanical findings, and review evidence against criteria and project design rules. There is no image-count limit. Include unchanged captures when a criterion needs them; use crops and omit redundant evidence. Keyboard actions can run without images, and filmstrips are for changed motion. Rerun affected pathways after fixes, expanding to every consumer of changed shared styles/tokens. Preserve ownership, head binding, carry-forward records, and the two-round escalation.
...
SDD File Handoffs: before dispatching a rendering task, the orchestrator reads ux-gate’s `matrix.md` and optional project policy, writes `.toolbelt/ux/smoke/task-<N>/matrix.json`, and fills `[UX_SMOKE_COMMAND]` with the absolute script, matrix, output, and project-root paths. (The reviewer-side rule — a missing UI smoke entry on a rendering diff is Important — lands in Task 18, which owns the reviewer prompts.)
...
- Per-task smoke matrix (ruling from Task 15's review): the orchestrator writes `.toolbelt/ux/smoke/task-<N>/matrix.json` for a task whose Files render anything — one pathway covering the task's surface, Launch/Auth/Theme from `.toolbelt/ux-policy.md` — and passes its path in `[UX_SMOKE_COMMAND]`. Preserve the File Handoffs instruction added in Task 15; do not add a duplicate. The orchestrator owns the matrix.
...
delivery Step 4: "When the boundary meets ux-gate’s entry condition or UX review is explicitly requested, tell SDD the boundary is UX-gated; SDD runs ux-gate at the final gate." Keep "That broad final review is the slice gate; do not add another whole-slice review."
