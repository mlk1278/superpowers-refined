...
Write the matrix to `.toolbelt/ux/matrix.json`, then run `scripts/ux-capture` from this skill's directory with `--out .toolbelt/ux/head/` and `--baseline .toolbelt/ux/baseline/` — interactive-design's prototype captures when they are there, otherwise capture the base branch from the same matrix into that directory. Stills are named `<pathway>-<step>-<state>-<width>-<theme>.png`. A step the script cannot complete — auth, data, a dead route — is a finding, not a skip.
