...
**3. Dispatch the reviewer** on the `reviewer` route (specialty `code`) from the session routing brief, filling the template at [code-reviewer.md](code-reviewer.md). Pass the author's model so the resolver can keep the review independent.

Do not read `docs/REVIEW-GUIDANCE.md` yourself. The reviewer template loads it when it exists, along with the smell baseline at [smell-baseline.md](smell-baseline.md). Supply only concise nuance from the approved requirements and concrete risks; use `None` when there is none.
