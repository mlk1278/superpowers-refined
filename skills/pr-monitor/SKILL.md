...
3. Await, and at most once per head request each policy-named provider. Only a current-head review object or an authenticated completion naming the current commit counts as completion; acknowledgements and reactions never do. Exception: a review object naming the recorded docs-only carry-forward predecessor head counts as current-head completion — do not request a new provider review for a docs-only push.
